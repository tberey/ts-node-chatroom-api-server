...
|Version 0.4.2 [2020-03-26]|<ul><li>Add check/feature to registering, requiring the user to confirm chosen password when signing up, and accompanying client-side error handling (not necessary to be done on server-side, as it is not damaging/deleting any assets if the user is able to skip this check).</li><li>Update README.md</li></ul>|